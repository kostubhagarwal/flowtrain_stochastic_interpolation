...
class ConditionalPosteriorFlowSolver:
    """ODE solver that augments a borehole-conditional prior velocity with gravity guidance.

    Args:
        net:                trained velocity-prediction network (Unet3DCondV3)
        ...
        borehole_mask:      boolean tensor (X, Y, Z) marking observed borehole voxels —
                            excluded from gravity gradient (prior owns those)
        method:             'euler' | 'rk4'
        grad_clip:          if set, clip the norm of the likelihood gradient to this value
        normalize_grad:     scale gradient so its norm equals the prior velocity norm
    ...
    def __init__(
        ...
        self._borehole_mask = (
            borehole_mask.bool().to(self.device).view(1, 1, *borehole_mask.shape[-3:])
            if borehole_mask is not None else None
        )
    ...
    def solve(
        ...
        Args:
            ...
            save_trajectory: if True return (n_steps+1, B, E, X, Y, Z), else just final (B, E, X, Y, Z)
        """
        dt      = (tf - t0) / n_steps
